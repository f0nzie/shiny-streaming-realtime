...
getStock <- function(url){
  ...
  # print(names(stock_data))
  .rs <- stock_data$rows$row %>%
    arrange(desc(id))             # sort by record id
  return(.rs)
}

# Transaction time check function
checkTime <- function(){
  this_time <- function(){
...
plotStock <- function(input, stock_data, limit = TRUE, n, sub_title){
  ...
  hc <- highchart() %>%
    ...
    hc_yAxis_multiples(
      list(title = list(text = "Price (x1000 VND)"),
           align = "left",
           showFirstLabel = FALSE,
           showLastLabel = FALSE,
           marker = list(enabled = FALSE)
      ),
      list(title = list(text = "Shares Volume (x10)"),
           align = "righ",
           showFirstLabel = FALSE,
           showLastLabel = FALSE,
           opposite = TRUE
    ...
    hc_add_series(name = "Volume of shares traded", data = stock_data$vol/100, type = "column", yAxis = 1) %>%
    hc_title(text = "SHI shares") %>%
    hc_subtitle(text = sub_title, style = list(color = "#2b908f", fontWeight = "bold")) %>%
    hc_add_series(name = "Price", data = stock_data$price, type = "spline")
...
shinyServer(function(input, output) {
  ...
  st_dt <- getStock(url)
  # print(st_dt)
  sub_title <- paste0("Updated at ", now(tzone = "Asia/Ho_Chi_Minh"))
  ...
  output$stockPlot <- renderHighchart({
    ...
    invalidateLater(60000) #1 update once every minute. 60 seconds = 60000 ms
    if (checkTime()) {
