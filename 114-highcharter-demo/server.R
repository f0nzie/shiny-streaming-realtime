...
function(input, output) {
  ...
  output$highscatter <- renderHighchart({
    ...
    hc_add_series(data = mtcars, type = "scatter", name = "mtcars",
                  hcaes(x = wt, 
                        y = mpg,
                        color = drat, 
                        size = hp,
  ...
  output$highohlc <- renderHighchart({
    ...
    hcbase() %>% 
      hc_add_series(as.xts(sample_matrix))
  ...
  output$tsforecast <- renderHighchart({
    ...
    highchart() %>% 
      hc_add_series(object$x, name = input$ts) %>% 
  ...
  output$pluginsfa <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_add_series(data = mtcars, type = "scatter", mapping = hcaes(x = mpg[1:16], 
                                                                     y = disp[1:16]),
                            marker = list(symbol = "text:\\\uf1b9"),
                            icon = as.character(shiny::icon("car")),
                            name = "cars", showInLegend = TRUE) %>% 
      hc_add_series(data = mtcars, type = "scatter", mapping = hcaes(x = mpg[17:32], 
                                                                     y = disp[17:32]),
                            marker = list(symbol = "text:\\\uf1ba"),
